...
export const EditContact = () => {
    ...
    const [formData, setFormData] = useState(store.agenda.find(el => el.id == contactInfo.id));
    ...
    const handleReset = e => {
        e.preventDefault();
        setFormData(store.agenda.find(el => el.id == contactInfo.id))
    }

    const handleSubmit = e => {
        e.preventDefault();
        contactServices.editContact('Albatroxs', contactInfo.id, formData)
        navigate('/')
    }
    ...
    return (
        <div className="container text-center mt-4 bg">
            <h1 className="mb-4">Edit contact</h1>
            <form onSubmit={handleSubmit} className="form-control bg-primary">
                ...
                <input className={"form-control mb-2"} onChange={handleChange} value={formData.email} placeholder="type your email" type="text" name="email" />
                ...
                <button className={"btn btn-light me-2 text-warning"} onClick={handleReset} >Reset</button>
                <button className={"btn btn-light me-2 text-success"} onClick={handleDeleteInfo} name=""> Delete </button>
